...
def get_last_page():
    ...
    for link in links[:-1]: #pages = pages[0:-1] 한 번에 넣어서 마지막 요소 없앰으로서 에러 방지. 즉, 모든 span들을 가져오되, 마지막 것은 제한다. ex) pages[0:5]는 0부터 시작해서 5까지 가져오는 것
...
def extract_jobs(last_page):
    ...
    result = requests.get(f"{URL}&start={page*LIMIT}") #우리가 원하는 형태의 url로 request도 가능! 
...
def get_jobs():
    ...
    return jobs # 모든 구직광고 저장된 List 반환